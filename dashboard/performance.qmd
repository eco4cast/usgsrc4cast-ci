...
aquatics_focal_sites <- c("USGS-05553700", "USGS-01463500",
                          # two representative CA DWR sites
                          "DWR-SJR", "DWR-MLS")
```
...
df_P1D <- arrow::open_dataset("../cache/summaries/duration=P1D") |>
  ...
  collect() |>
  # exclude persistenceRW: its performance is so poor it obscures trends in other models
  filter(model_id != "persistenceRW") |>
  # exclude example models
  filter(!grepl("example", model_id, ignore.case = TRUE))
...
target_reference_datetime_P1D <- Sys.Date() - lubridate::days(30)

ex_P1D <- df_P1D_scores |>
  filter(lubridate::as_date(reference_datetime) == target_reference_datetime_P1D)
...
df_P1D |>
  ...
  forecast_plots(ncol = 2, height_svg = 7)
